...
public class ProveedorDAO {
    ...
    /**
     * Método para obtener todos los NOMBRES de proveedores de la base de datos.
     *
     * @return Lista de objetos ProveedorModel
     */
    public List<String> obtenerNombresTodosProveedores() {
        String sql = "SELECT NombreComercial FROM Proveedor WHERE Estado = 1";
        ...
        try (Connection connection = DataBase.nDataBase().getConnection();
            ...
            while (rs.next()) {
                ...
                proveedores.add(proveedor.getNombreComercial());
            }
        } catch (SQLException e) {
            System.err.println("Error al obtener nombres de proveedores: " + e.getMessage());
        }
     ...
     *
     * @param id El ID del proveedor a obtener
     * @return El objeto NombreProveedor si se encuentra, nada si no se encuentra
     */
    public String obtenerNombreProveedorPorId(int id) {
        String sql = "SELECT NombreComercial FROM Proveedor WHERE IdProveedor = ?";
        ...
        try (Connection connection = DataBase.nDataBase().getConnection();
            ...
            try (ResultSet rs = statement.executeQuery()) {
                if (rs.next()) {
                    NombreProveedor = rs.getString("NombreComercial");
                    System.err.print(NombreProveedor);
                }
            }
        } catch (SQLException e) {
            System.err.println("Error al obtener el nombre de proveedor: " + e.getMessage());
        }
