...
public class ClienteDAO {
    ...
    /**
     * Método para obtener todos los NITS de clientes de la base de datos.
     *
     * @return Lista de objetos ClienteModel
     */
    public List<String> obteneTodosNits() {
        String sql = "SELECT Nit FROM Cliente WHERE Estado = 1";
        ...
        try (Connection connection = DataBase.nDataBase().getConnection();
            ...
            while (rs.next()) {
                ClienteModel nit = new ClienteModel(
                        rs.getString("Nit"),
                        rs.getInt("IdPersona")
                );
                nits.add(nit.getNit());
            ...
            System.err.println("Error al obtener todos los nit: " + e.getMessage());
        }
    ...
    /**
     * Método para obtener el NIT de un cliente por su ID.
     *
     * @param id El ID del cliente a obtener
     * @return El NIT de cliente si se encuentra, nada si no se encuentra
     */
    public String obtenerNitClientePorId(int id) {
        String sql = "SELECT Nit FROM Cliente WHERE IdCliente = ?";
        String NitCliente = "";
        ...
        try (Connection connection = DataBase.nDataBase().getConnection();
            ...
            try (ResultSet rs = statement.executeQuery()) {
                if (rs.next()) {
                    NitCliente = rs.getString("Nit");
                    System.err.print(NitCliente);
                }
            }
        } catch (SQLException e) {
            System.err.println("Error al obtener el nit del cliente: " + e.getMessage());
        }
        return NitCliente;
    }
    /**
