...
public class StockDAO {
    ...
    public StockModel obtenerStockPorId(int idStock) {
        ...
        try (Connection connection = DataBase.nDataBase().getConnection();
            ...
            try (ResultSet rs = statement.executeQuery()) {
                if (rs.next()) {
                    ...
                    stock.setIdStock(rs.getInt("IdStock")); // Seteamos el ID del producto
                    stock.setCantidadDisponible(rs.getInt("CantidadDisponible")); // Setear cantidad disponible
                    stock.setFechaIngreso(rs.getDate("FechaIngreso")); // Setear la fecha de ingreso
                    stock.setEstado(rs.getBoolean("Estado")); // También seteamos el Estado
                }
            }
        } catch (SQLException e) {
        ...
        return stock; // Retorna el stock encontrado o null
    }
    ...
    public List<StockModel> obtenerTodosLosStocks() {
        ...
        try (Connection connection = DataBase.nDataBase().getConnection();
            ...
            while (rs.next()) {
                ...
                stock.setIdStock(rs.getInt("IdStock")); // Seteamos el ID del stock
                stock.setCantidadDisponible(rs.getInt("CantidadDisponible")); // Setear la cantidad disponible
                stock.setFechaIngreso(rs.getDate("FechaIngreso")); // Setear la fecha de ingreso
                stock.setEstado(rs.getBoolean("Estado")); // También seteamos el estado
                stocks.add(stock); // Agregamos el producto a la lista
            }
        } catch (SQLException e) {
